    ...
    public class PanoramicSqlDAL : IPanoramicDAL
    {
        ...
        public List<PanoramicModel> GetAllPanoramics()
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    SqlCommand cmd = new SqlCommand(@"SELECT * FROM panoramics ORDER BY panoramic_name;", conn);
        ...
        public PanoramicModel GetPanoramicById(int id)
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    SqlCommand cmd = new SqlCommand(@"SELECT panoramics.panoramic_id FROM panoramics INNER JOIN trails ON panoramics.trail_id = trails.trail_id WHERE panoramics.panoramic_id = @id;", conn);
        ...
        public List<PanoramicModel> GetPanoramicsByTrailId(int trailId)
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    SqlCommand cmd = new SqlCommand(@"SELECT * FROM panoramics INNER JOIN trails ON panoramics.trail_id = trails.trail_id;", conn);
